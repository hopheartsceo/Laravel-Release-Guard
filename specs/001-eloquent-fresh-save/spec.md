...
- **FR-011**: Regression coverage for the eventual implementation MUST begin with failing tests for new or broadened behavior: positive supported fresh-instance `save()` detection, and DB005 `WARNING` / `UNKNOWN` classification for fresh-save evidence. Conservative negative boundaries MUST be captured before production changes as baseline characterization/regression tests, including loaded-model `save()` remaining non-insert, ambiguous provenance remaining outside fresh-save findings, source-order/reassignment/control-flow exclusions, and existing v0.1 behavior remaining preserved; these baseline safety tests are expected to pass on v0.1 where the behavior is already safe.
